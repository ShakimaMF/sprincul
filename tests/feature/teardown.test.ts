...
describe("Sprincul - Teardown", () => {
	...
	test("destroy(modelName, element) tears down only the matching instance", async () => {
      ...
      `;
		...
		const secondButton = secondRoot.querySelector(
			"button",
		) as HTMLButtonElement;
		const firstCount = firstRoot.querySelector(
			"[data-bind-count]",
		) as HTMLElement;
		const secondCount = secondRoot.querySelector(
			"[data-bind-count]",
		) as HTMLElement;
	...
	test("body-level observer reacts to parent removal and calls destroy for removed model roots", async () => {
      ...
      `;
		...
		const observedRoot = container.querySelector(
			"#observed-root",
		) as HTMLElement;
		...
		const matchingCall = destroySpy.mock.calls.find(([name, element]) => {
			return (
				name === "ObservedModel" &&
				(element as HTMLElement | undefined)?.id === observedRoot.id
			);
		});
